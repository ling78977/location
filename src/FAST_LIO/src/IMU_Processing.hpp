...
inline void ImuProcess::UndistortPcl(
  ...
  pcl_out = *(meas.lidar);
  // std::cout<<"点云数量："<<pcl_out.points.size()<<std::endl;
  ...
  // std::cout<<"第yi次循环耗时： "<<omp_get_wtime()-t1<<std::endl;
  ...
  for (auto it_kp = IMUpose.end() - 1; it_kp != IMUpose.begin(); it_kp--) {
    ...
    for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--) {
      ...
      // std::cout<<"矩阵相乘耗时1： "<<omp_get_wtime()-tt<<std::endl;
      ...
      // std::cout<<"矩阵运算： "<<omp_get_wtime()-t2<<"循环了
      // "<<num<<"次"<<std::endl; std::cout<<"矩阵相乘耗时：
      // "<<omp_get_wtime()-t2<<std::endl; save Undistorted points and their
      // rotation
    ...
    // std::cout<<"一个位姿之间的点数： "<<num<<std::endl;
  }
}
...
inline void ImuProcess::publish_odometry(
    const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr
        pubOdomAftMapped,
    std::unique_ptr<tf2_ros::TransformBroadcaster> &tf_br,
    esekfom::esekf<state_ikfom, 12, input_ikfom> &kf,
  ...
  odomAftMapped.pose.pose.position.x = kf.get_x().pos(0);
  odomAftMapped.pose.pose.position.y = kf.get_x().pos(1);
  odomAftMapped.pose.pose.position.z = kf.get_x().pos(2);
  odomAftMapped.pose.pose.orientation.x = kf.get_x().rot.x();
  odomAftMapped.pose.pose.orientation.y = kf.get_x().rot.y();
  odomAftMapped.pose.pose.orientation.z = kf.get_x().rot.z();
